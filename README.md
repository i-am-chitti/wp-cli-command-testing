...
- **`WP_VERSION=latest` isn't a value `wp core download --version=` accepts.** `wp-cli-tests`' `run-behat-tests` wrapper quietly resolved it via `curl` + `jq` first, so calling `vendor/bin/behat` directly breaks on it. Rather than reimplement that resolution, this repo just pins concrete WordPress versions everywhere — in `.env` and in the CI matrix. A pinned matrix is more reproducible anyway: a green build can't turn red overnight because WordPress shipped a release.
